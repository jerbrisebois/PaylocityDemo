...
Other notes: This seems inconsistent. Was reproduced repeatedly in Firefox on 7/31 but could not reproduce on 8/1. I am unsure if this has to do with something changing in the environment.
...
When pressing "Add" to submit an employee, the user may be able to click the button multiple times before the popup can close, resulting in multiple entries for the same employee to populate the table.
This may only be reproducible with tooling to click rapidly or in poor network conditions where latency prevents the page from updating before the user can click again. For this test, throttling was introduced in the browser to expose the issue.

Steps to reproduce:
Log into the application
Click "Add Employee"
Enter valid values for first name, last name and dependents.
Click "Add" rapidly*
...
Steps to reproduce:
Log into the application in two instances (multiple browser windows or tabs)
Delete an existing employee in one instance
Edit the deleted employee in the second instance. Press "Update"
